(function () {
  ...
  style.innerHTML = `
    ...
    /* 3 Mode Tabs (AUTO / INSTANT / SIGNAL) */
    ...
    /* ⚡ 1-Click Instant AI Trade Button */
    .zyro-btn-instant {
  ...
  // 3 Modes handling
  let activeMode = 'auto'; // 'auto', 'instant', 'manual'
  ...
  function setMode(mode) {
    ...
    if (mode === 'instant') {
      ...
      speakVoice("Instant Analysis Mode Activated");
    } else {
      instantBtn.style.display = 'block';
  ...
  function getLiveRealPrice() {
    // 1. Direct Quotex current price elements
    ...
    // 2. Scan visible small text nodes across chart containers
    ...
    // 3. Fallback to last known price or synthetic reference so trade NEVER gets blocked
  ...
  function analyzeHumanTradingDecision(curPrice) {
    ...
    // RULE 1: BULLISH / BEARISH ENGULFING
    ...
    // RULE 2: PINBAR REVERSAL AT SUPPORT / RESISTANCE
    ...
    // RULE 3: THREE CONSECUTIVE CANDLES MOMENTUM
  ...
  // --- 4. Execution & Strict Real Win/Loss Logic ---
  let isRunning = false;
  let baseAmount = 1;
  let currentAmount = 1;
  ...
  // --- ⚡ 1-CLICK INSTANT AI TRADE HANDLER ---
  instantBtn.onclick = () => {
    if (isWaitingResult) {
      alert("Wait for the current trade to finish!");
      return;
    }

    let curP = getLiveRealPrice();
    if (!curP) {
      curP = lastKnownPrice || 1.08450;
    }

    baseAmount = parseFloat(document.getElementById('zyroAmt').value) || 1;
    if (currentStep === 0) currentAmount = baseAmount;

    // Instant deep scan and analysis
    const analysis = analyzeHumanTradingDecision(curP);
