(function () {
  ...
  if (document.getElementById('zyro-algo-container')) {
    alert('⚡ ZYRO ELITE PRO is already active on screen!');
    return;
  }
  ...
  style.innerHTML = `
    ...
    #zyro-algo-container {
      position: fixed; top: 50px; right: 10px; width: 250px;
  ...
  container.innerHTML = `
    <div class="zyro-header" id="zyroDrag">
      <div class="zyro-brand">⚡ <span>ZYRO</span> ELITE PRO</div>
    ...
    <div class="zyro-body">
      ...
      <div class="zyro-signal-card" id="zyroSignalCard">
        <div class="zyro-confluence" id="zyroConfluence">PRICE ACTION + CANDLE BODY</div>
        <div class="zyro-direction" id="zyroDir" style="color:#00f2fe;">READY FOR NEXT CANDLE</div>
      </div>
      ...
      <div class="zyro-stats-grid">
        ...
        <div class="zyro-stat-item"><span>Candle:</span><b id="zyroCandleStatus" style="color:#38bdf8;">SYNCING</b></div>
      </div>
      ...
      <button class="zyro-btn zyro-btn-start" id="zyroToggleBtn">START ZYRO ELITE</button>
    </div>
  ...
  // --- 3. HIGH ACCURACY CANDLE BODY & PRICE ACTION ENGINE ---
  let candleOpenPrice = null;
  let candleHighPrice = null;
  let candleLowPrice = null;
  let lastEvaluatedCandleColor = null;

  function trackCandleMetrics(currentPrice, second) {
    if (second <= 1 || candleOpenPrice === null) {
  ...
  function evaluateHighAccuracyPriceAction(currentPrice) {
    if (!candleOpenPrice || !candleHighPrice || !candleLowPrice) {
      return { direction: "CALL", confluenceText: "TREND CONTINUATION", quality: "GOOD" };
    }

    const open = candleOpenPrice;
    const close = currentPrice;
    const high = candleHighPrice;
    const low = candleLowPrice;
    ...
    let direction = isGreen ? "CALL" : "PUT";
    let confluenceText = isGreen ? "Bullish Candle Flow" : "Bearish Candle Flow";

    // Reversal Pinbar Filter (Shooting star / Hammer)
    if (isGreen && upperWick > bodySize * 1.6) {
      // Massive upper rejection -> Sell Reversal
      direction = "PUT";
      confluenceText = "Bearish Pinbar Rejection";
    } else if (!isGreen && lowerWick > bodySize * 1.6) {
      // Massive lower rejection -> Buy Reversal
      direction = "CALL";
      confluenceText = "Bullish Hammer Rebound";
    } else if (bodySize > totalRange * 0.55) {
      // Strong Momentum Candle Continuation
      direction = isGreen ? "CALL" : "PUT";
      confluenceText = isGreen ? "Bullish Momentum Push" : "Bearish Momentum Push";
    }
  ...
  const candleStatusEl = document.getElementById('zyroCandleStatus');
  ...
  toggleBtn.onclick = () => {
    ...
    if (isRunning) {
      ...
      toggleBtn.innerText = 'STOP ZYRO ELITE';
      playSoundFX('sniper');
      speakVoice("Zyro Elite Activated");
    } else {
      toggleBtn.className = 'zyro-btn zyro-btn-start';
      toggleBtn.innerText = 'START ZYRO ELITE';
  ...
  function executeSniperTrade(direction, confluenceText) {
    ...
    if (isAutoMode) {
      if (direction === 'CALL' && upBtn) {
        upBtn.click();
        playSoundFX('sniper');
        console.log(`[ZYRO ELITE] Executed CALL at ${entryPrice} ($${currentAmount})`);
      } else if (direction === 'PUT' && downBtn) {
        downBtn.click();
        playSoundFX('sniper');
        console.log(`[ZYRO ELITE] Executed PUT at ${entryPrice} ($${currentAmount})`);
      }
    } else {
  ...
  setInterval(() => {
    ...
    if (curP !== null) {
      trackCandleMetrics(curP, s);

      // Display live candle state
      if (candleOpenPrice !== null) {
        const isBull = curP >= candleOpenPrice;
        candleStatusEl.innerText = isBull ? "▲ GREEN" : "▼ RED";
        candleStatusEl.style.color = isBull ? "#00e676" : "#ef4444";
      }
    }

    // Asset switch tracking
    ...
    if (detectedAsset && detectedAsset !== currentActiveAsset) {
      ...
      cardDir.innerText = 'SYNCING CANDLE...';
    ...
    // Phase 1: Real Price Action Analysis between 45s and 56s
    if (s >= 45 && s <= 56) {
      let analysis = evaluateHighAccuracyPriceAction(curP);

      // If we are in Martingale Step 1 or 2, follow recovery flow!
      if (currentStep > 0 && lastTradeDirection) {
        analysis.direction = lastTradeDirection;
        analysis.confluenceText = `Martingale Recovery Step ${currentStep}`;
      }
    ...
    // Phase 2: Precision Execution at 58 seconds
      ...
      executeSniperTrade(analysis.direction, analysis.confluenceText);
    } 
    else if (s < 45) {
      cardConfluence.innerText = `${currentActiveAsset} CANDLE SYNC`;
      cardDir.innerText = 'ANALYZING PRICE ACTION...';
      cardDir.style.color = '#00f2fe';
    }
  }, 1000);

  speakVoice("Zyro Elite Pro Ready");
})();
