(function () {
  ...
  // --- 1. Realistic Audio FX Synthesis ---
  ...
  style.innerHTML = `
    ...
    #zyro-algo-container {
      position: fixed; top: 50px; right: 10px; width: 255px;
  ...
  container.innerHTML = `
    <div class="zyro-header" id="zyroDrag">
      <div class="zyro-brand">⚡ <span>ZYRO</span> HUMAN AI PRO</div>
    ...
    <div class="zyro-body">
      ...
      <div class="zyro-signal-card" id="zyroSignalCard">
        <div class="zyro-confluence" id="zyroConfluence">HUMAN PATTERN ANALYSIS READY</div>
        <div class="zyro-direction" id="zyroDir" style="color:#00f2fe;">READING CANDLES</div>
      </div>
      ...
      <div class="zyro-stats-grid">
        ...
        <div class="zyro-stat-item"><span>Pattern:</span><b id="zyroPatternText" style="color:#38bdf8;">SYNCING</b></div>
      </div>
      ...
      <button class="zyro-btn zyro-btn-start" id="zyroToggleBtn">START HUMAN AI</button>
    </div>
  ...
  function showResultPopup(isWin, deltaText) {
    ...
    if (isWin) {
      popup.innerHTML = `🎉 ALHAMDULLIAH! PROFIT! 💸<br><span style="font-size:9px; color:#a7f3d0;">${deltaText}</span>`;
      playSoundFX('win');
      speakVoice("Alhamdulillah! Profit booked!");
    } else {
      popup.innerHTML = `⚠️ SORRY! LOSS DETECTED...<br><span style="font-size:9px; color:#fca5a5;">${deltaText}</span>`;
      playSoundFX('loss');
  ...
  // --- 3. 🧠 HUMAN-LIKE MULTI-CANDLE PRICE ACTION & PATTERN ENGINE ---
  let candleHistory = []; // Stores completed real candles
  ...
  function analyzeHumanTradingDecision(curPrice) {
    if (curOpen === null || curHigh === null || curLow === null) {
      return { direction: "CALL", pattern: "MARKET CONTINUATION", confidence: 85 };
    }

    const curIsGreen = curPrice >= curOpen;
    const curBody = Math.abs(curPrice - curOpen);
    const curTotal = Math.max(curHigh - curLow, 0.00001);
    const curUpperWick = curHigh - Math.max(curOpen, curPrice);
    const curLowerWick = Math.min(curOpen, curPrice) - curLow;

    // Check for previous completed candle if available
    const prev = candleHistory.length > 0 ? candleHistory[candleHistory.length - 1] : null;

    // --- RULE 1: BULLISH / BEARISH ENGULFING (Human Favorite #1) ---
    if (prev && prev.body > 0.00005) {
      // Bullish Engulfing: previous was red, current is large green swallowing previous body
      if (!prev.isGreen && curIsGreen && curBody > prev.body * 1.15 && curPrice > prev.high) {
        return { direction: "CALL", pattern: "BULLISH ENGULFING", confidence: 94 };
      }
      // Bearish Engulfing: previous was green, current is large red swallowing previous body
      if (prev.isGreen && !curIsGreen && curBody > prev.body * 1.15 && curPrice < prev.low) {
        return { direction: "PUT", pattern: "BEARISH ENGULFING", confidence: 94 };
      }
    }

    // --- RULE 2: PINBAR REVERSAL AT SUPPORT / RESISTANCE (Human Favorite #2) ---
    // Hammer (Long lower shadow -> Buyers took over)
    if (curLowerWick > curBody * 1.8 && curUpperWick < curBody * 0.6) {
      return { direction: "CALL", pattern: "BULLISH HAMMER REBOUND", confidence: 91 };
    }
    // Shooting Star (Long upper shadow -> Sellers took over)
    if (curUpperWick > curBody * 1.8 && curLowerWick < curBody * 0.6) {
      return { direction: "PUT", pattern: "SHOOTING STAR REJECTION", confidence: 91 };
    }

    // --- RULE 3: THREE CONSECUTIVE CANDLES MOMENTUM (Trend Flow) ---
    if (candleHistory.length >= 2) {
      const c1 = candleHistory[candleHistory.length - 2];
      const c2 = candleHistory[candleHistory.length - 1];
      if (c1.isGreen && c2.isGreen && curIsGreen && curBody > 0.0001) {
        return { direction: "CALL", pattern: "3 BULLISH SOLDIERS", confidence: 89 };
      }
      if (!c1.isGreen && !c2.isGreen && !curIsGreen && curBody > 0.0001) {
        return { direction: "PUT", pattern: "3 BLACK CROWS DOWNTREND", confidence: 89 };
      }
    }

    // --- RULE 4: BREAKOUT & TREND CONTINUATION ---
    if (curBody > curTotal * 0.6) {
      return {
        direction: curIsGreen ? "CALL" : "PUT",
        pattern: curIsGreen ? "BULLISH BREAKOUT PUSH" : "BEARISH BREAKOUT PUSH",
        confidence: 88
      };
    }

    // --- DEFAULT SMART FOLLOWING ---
    return {
      direction: curIsGreen ? "CALL" : "PUT",
      pattern: curIsGreen ? "SMART BULLISH FLOW" : "SMART BEARISH FLOW",
      confidence: 85
    };
  }

  // --- 4. Execution & 100% Strict Real Win/Loss Logic ---
  let isRunning = false;
  let baseAmount = 1;
  let currentAmount = 1;
  ...
  const patternTextEl = document.getElementById('zyroPatternText');
  ...
  toggleBtn.onclick = () => {
    ...
    if (isRunning) {
      ...
      toggleBtn.innerText = 'STOP HUMAN AI';
      playSoundFX('sniper');
      speakVoice("Zyro Human AI Activated");
    } else {
      toggleBtn.className = 'zyro-btn zyro-btn-start';
      toggleBtn.innerText = 'START HUMAN AI';
  ...
  function executeSniperTrade(direction, patternName) {
    ...
    if (isAutoMode) {
      if (direction === 'CALL' && upBtn) {
        upBtn.click();
        playSoundFX('sniper');
        console.log(`[ZYRO HUMAN] Executed CALL at ${entryPrice} ($${currentAmount}) - ${patternName}`);
      } else if (direction === 'PUT' && downBtn) {
        downBtn.click();
        playSoundFX('sniper');
        console.log(`[ZYRO HUMAN] Executed PUT at ${entryPrice} ($${currentAmount}) - ${patternName}`);
      }
    } else {
      speakVoice(`Human Signal: ${direction}! ${patternName}`);
    }
    ...
    setTimeout(() => {
      ...
      // Pure Mathematics: CALL WIN = exit > entry, PUT WIN = exit < entry
      ...
      if (netProfit <= -stopLossLimit) {
        alert(`⚠️ STOP LOSS HIT (-$${stopLossLimit}). Zyro Stopped to protect capital!`);
        toggleBtn.click();
  ...
  setInterval(() => {
    ...
    if (curP !== null) {
      ...
      // Archive candle at second 59
      if (s === 59) {
        archiveCompletedCandle(curP);
      }
    }

    // Asset switch auto detection
    ...
    if (detectedAsset && detectedAsset !== currentActiveAsset) {
      ...
      cardDir.innerText = 'READING NEW CANDLES...';
    ...
    // Scan phase: 45s to 56s
    if (s >= 45 && s <= 56) {
      let analysis = analyzeHumanTradingDecision(curP);

      // Follow Martingale Step 1 or 2 recovery
      ...
      patternTextEl.innerText = analysis.pattern.slice(0, 14);
    ...
    // Execution at exactly 58 seconds
      ...
      cardConfluence.innerText = `${currentActiveAsset} CANDLE HISTORY: ${candleHistory.length}`;
      cardDir.innerText = 'WATCHING MARKET STRUCTURE...';
      cardDir.style.color = '#00f2fe';
    }
  }, 1000);

  speakVoice("Zyro Human AI Ready");
})();
