(function () {
  ...
  if (document.getElementById('zyro-algo-container')) {
    alert('⚡ ZYRO 3.0 PRO is already active on screen!');
    return;
  }
  ...
  container.innerHTML = `
    <div class="zyro-header" id="zyroDrag">
      <div class="zyro-brand">⚡ <span>ZYRO</span> 3.0 SNIPER</div>
    ...
    <div class="zyro-body">
      ...
      <div class="zyro-signal-card" id="zyroSignalCard">
        <div class="zyro-confluence" id="zyroConfluence">SCANNING LIVE CANDLES</div>
  ...
  // Result popup with clear real delta verification
  ...
  // --- 3. LIVE ASSET DETECTOR & SWITCH TRACKER ---
  ...
  // --- 5. ULTRA-ACCURATE MULTI-STRATEGY ENGINE ---
  let priceTicks = [];
  function recordCurrentPrice() {
    const p = getLiveRealPrice();
    if (p !== null) {
      priceTicks.push(p);
      if (priceTicks.length > 70) priceTicks.shift();
    }
  }
  ...
  function evaluateUltraSignal() {
    ...
    if (p.length >= 8) {
      ...
      // Strategy 1: Momentum Micro Flow
      ...
      // Strategy 2: Liquidity Wick Sweep Rejection
      if (prev <= low && cur > low) {
        buyScore += 35;
        confluences.push("Buy-Side Liquidity Rebound");
      } else if (prev >= high && cur < high) {
        sellScore += 35;
        confluences.push("Sell-Side Liquidity Drop");
      }

      // Strategy 3: EMA 5 vs EMA 15 Fast Cross
  ...
  // --- 6. Execution & 100% Guaranteed Correct Win/Loss Logic ---
  let isRunning = false;
  let baseAmount = 1;
  let currentAmount = 1;
  ...
  function executeSniperTrade(direction, confluenceText) {
    ...
    // After 60 seconds (expiry), verify exact win/loss
    setTimeout(() => {
      const exitPrice = getLiveRealPrice();
      let isWin = false;

      // 100% PURE MATHEMATICAL VERIFICATION:
      // CALL WIN: Exit Price MUST be greater than Entry Price (> entryPrice)
      // PUT WIN: Exit Price MUST be lower than Entry Price (< entryPrice)
      ...
      const deltaText = `Entry: ${entryPrice} → Exit: ${exitPrice} (Δ ${diff})`;
  ...
  // --- Main 1-Second Loop (Market Switch Detection + Auto Scan) ---
  setInterval(() => {
    recordCurrentPrice();

    // 1. Detect if User switched currency pair
    ...
    if (detectedAsset && detectedAsset !== currentActiveAsset) {
      ...
      // Reset buffer and scan immediately for the new market!
      priceTicks = [];
      cardConfluence.innerText = `NEW PAIR: ${currentActiveAsset}`;
      cardDir.innerText = 'SCANNING NEW MARKET...';
    ...
    // Execution at exactly 57 seconds
      ...
      cardConfluence.innerText = `${currentActiveAsset} LIVE FEED`;
      cardDir.innerText = 'MONITORING CANDLES...';
      cardDir.style.color = '#00f2fe';
    }
  }, 1000);

  speakVoice("Zyro 3.0 Ready");
})();
